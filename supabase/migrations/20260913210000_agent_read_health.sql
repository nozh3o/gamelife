-- Ассистент читает показатели Apple Health: срез «health».
--
-- Что изменилось против 20260913041500: добавлена одна ветка p_what='health',
-- которая отдаёт строки из health_days за период. Остальное тело функции
-- перенесено без единого изменения — включая счёт дат по Etc/GMT-5.
--
-- Умолчание периода своё: день — слишком узко для вопроса «как прошла
-- неделя», полгода замеров здесь не нужны. Две недели — это 14 строк,
-- достаточно, чтобы увидеть ритм, и дёшево по размеру ответа.
--
-- watchWorn отдаётся вместе с цифрами намеренно. Без него день, когда часы
-- лежали на зарядке, читается как день полного бездействия: пустые поля
-- и ноль выглядят одинаково только до тех пор, пока по ним не начали
-- делать выводы.
--
-- Известное ограничение: ветка стоит после раннего выхода на пустом
-- состоянии приложения (v_data is null). У человека, который ни разу
-- не открывал One, здоровье не прочитается. Переносить проверку выше —
-- это перетряхивать всю функцию ради случая, которого нет: строки
-- в health_days появляются только у того, у кого есть личный токен,
-- а токен выдаётся в самом приложении.
